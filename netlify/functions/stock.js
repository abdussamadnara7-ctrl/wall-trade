...
async function getStockData(ticker) {
  // ENGROH listed on PSX as ENGROH.KA (replaced ENGRO Jan 2025)
  ...
  try {
    ...
    // If Yahoo has no quote at all (new listing, data gap) — build from fallback
    if (!q) {
      const fb = PSX_FUNDAMENTALS[ticker];
      if (fb) {
        return {
          ticker, symbol,
          name:     ticker === 'ENGROH' ? 'Engro Holdings Ltd' : ticker,
          sector:   ticker === 'ENGROH' ? 'Holding Company' : 'Pakistan Stock Exchange',
          industry: ticker === 'ENGROH' ? 'Diversified Industrials' : '',
          price: null, change: null, changeAmt: null,
          high: null, low: null, prevClose: null, volume: null,
          week52High: null, week52Low: null, dir: 'up',
          dataSource: 'fallback',
          ...fb
        };
      }
    ...
    return {
      ...
      ticker,
      symbol,
      name:     q.longName || q.shortName || (ticker === 'ENGROH' ? 'Engro Holdings Ltd' : ticker),
      sector:   q.sector || (ticker === 'ENGROH' ? 'Holding Company' : 'Pakistan Stock Exchange'),
      industry: q.industry || (ticker === 'ENGROH' ? 'Diversified Industrials' : ''),
      ...
      // Fundamentals — Yahoo live value if present, else PSX_FUNDAMENTALS fallback
      ...(() => {
        const fb = PSX_FUNDAMENTALS[ticker] || {};
        const v = (live, fallback) => (live != null && live !== 'N/A' && live !== 'null') ? live : (fallback || 'N/A');
        return {
          marketCap:     q.marketCap ? 'Rs. ' + (q.marketCap / 1e9).toFixed(1) + 'B' : (fb.marketCap || 'N/A'),
          pe:            v(q.trailingPE?.toFixed(2), fb.pe),
          fwdPe:         v(q.forwardPE?.toFixed(2), fb.fwdPe),
          pb:            v(q.priceToBook?.toFixed(2), fb.pb),
          ps:            v(fmt(stats.priceToSalesTrailing12Months), 'N/A'),
          eps:           v(q.epsTrailingTwelveMonths?.toFixed(2), fb.eps),
          divYield:      q.trailingAnnualDividendYield ? (q.trailingAnnualDividendYield * 100).toFixed(2) + '%' : (fb.divYield || 'N/A'),
          roe:           v(pct(fin.returnOnEquity), fb.roe),
          roa:           v(pct(fin.returnOnAssets), fb.roa),
          grossMargin:   v(pct(fin.grossMargins), fb.grossMargin),
          opMargin:      v(pct(fin.operatingMargins), fb.opMargin),
          netMargin:     v(pct(fin.profitMargins), fb.netMargin),
          ebitda:        v(bil(fin.ebitda), fb.ebitda),
          revenue:       v(bil(fin.totalRevenue), fb.revenue),
          currentRatio:  v(fmt(fin.currentRatio), fb.currentRatio),
          quickRatio:    v(fmt(fin.quickRatio), fb.quickRatio),
          debtToEquity:  v(fmt(fin.debtToEquity), fb.debtToEquity),
          totalCash:     v(bil(fin.totalCash), fb.totalCash),
          totalDebt:     v(bil(fin.totalDebt), fb.totalDebt),
          fcf:           v(bil(fin.freeCashflow), fb.fcf),
          beta:          v(fmt(stats.beta), fb.beta),
          revenueGrowth:  v(pct(fin.revenueGrowth), fb.revenueGrowth),
          earningsGrowth: v(pct(fin.earningsGrowth), fb.earningsGrowth),
        };
      })(),
    };
  } catch(e) {
